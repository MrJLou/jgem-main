...
void main() async {
  ...
  DatabaseHelper.setDatabaseChangeCallback(
    ...
    try {
      ...
      final isRemoteChange = data != null &&
          (data.containsKey('source') || data.containsKey('clientInfo'));
      ...
      if (EnhancedShelfServer.isRunning && !isRemoteChange) {
        try {
          debugPrint(
              'MAIN: [HOST] Sending LOCAL change to clients: $table.$operation');
  ...
  Future.delayed(const Duration(seconds: 5), () async {
    ...
    final isClient =
        DatabaseSyncClient.isConnected && !EnhancedShelfServer.isRunning;
    final isHost = EnhancedShelfServer.isRunning;

    debugPrint(
        'SYNC DEBUG: Device status - isClient: $isClient, isHost: $isHost');
    debugPrint(
        'SYNC DEBUG: Client connected: ${DatabaseSyncClient.isConnected}');
    debugPrint(
        'SYNC DEBUG: Database callback set: ${DatabaseHelper.hasDatabaseChangeCallback()}');

    if (isClient) {
      debugPrint(
          'SYNC DEBUG: This is a CLIENT device, testing queue sync chain...');

      // Test 1: Verify database change callback works
      try {
        final queueItemId =
            'test-queue-item-${DateTime.now().millisecondsSinceEpoch}';
        final testData = {
        ...
        await DatabaseHelper.triggerDatabaseChangeCallback(
            'active_patient_queue', 'insert', queueItemId, testData);
        debugPrint(
            'SYNC DEBUG: TEST 1 - Database change callback test complete');
      } catch (e) {
        debugPrint(
            'SYNC DEBUG: TEST 1 FAILED - Database change callback error: $e');
      }
      ...
      debugPrint(
          'SYNC DEBUG: This is a HOST device, server should be ready for client connections');
    } else {
  ...
  DatabaseSyncClient.syncUpdates.listen((update) {
    if (update['type'] == 'session_invalidated' ||
      ...
      debugPrint(
          'MAIN: Session change detected from network: ${update['type']}');
        ...
        try {
          ...
          if (isLoggedIn) {
            ...
            if (username != null) {
              ...
              final hasConflicts =
                  await EnhancedUserTokenService.checkNetworkSessionConflicts(
                      username);
              if (hasConflicts) {
                debugPrint(
                    'MAIN: Session conflict detected, triggering validation');
  ...
  Timer.periodic(const Duration(minutes: 5), (timer) async {
    try {
      ...
      if (!isLoggedIn) {
        debugPrint(
            'MAIN: No active session after verification, skipping periodic sync');
        return;
      }
      ...
      if ((DatabaseSyncClient.isConnected || EnhancedShelfServer.isRunning)) {
        debugPrint(
            'MAIN: Triggering periodic session sync (every 5 minutes) - authenticated user detected');
        await CrossDeviceSessionMonitor.triggerImmediateSessionSync();
      }
    } catch (e) {
  ...
  Timer.periodic(const Duration(seconds: 10), (timer) async {
    try {
      if (!DatabaseHelper.hasDatabaseChangeCallback()) {
        ...
        // Re-register the main database change callback
        DatabaseHelper.setDatabaseChangeCallback(
            (table, operation, recordId, data) async {
          try {
            debugPrint(
        ...
        debugPrint('MAIN: Database callback restored successfully');
      }
    } catch (e) {
      debugPrint('MAIN: Error in callback restoration check: $e');
    }
  });
    ...
    if (serverIp != null) {
      ...
      try {
        final port = serverPort is int
            ? serverPort
            : (serverPort is String ? int.tryParse(serverPort) ?? 8080 : 8080);
        ...
        debugPrint(
            'Cached server $serverIp:$serverPort is no longer reachable: $e');
