...
class BTreeQueueManager {
  ...
  Future<void> initialize(QueueService queueService) async {
    ...
    try {
      ...
      _isInitialized = true;
      debugPrint('BTreeQueueManager: Initialized successfully with ${_bTreeQueue.size} items');
    } catch (e) {
  ...
  Future<void> _loadExistingQueueItems() async {
    try {
      List<ActivePatientQueueItem> existingItems = await _queueService.getActiveQueueItems();
      if (existingItems.isNotEmpty) {
        _bTreeQueue.buildFromList(existingItems);
        debugPrint('BTreeQueueManager: Loaded ${existingItems.length} existing queue items');
      }
    } catch (e) {
  ...
  void _ensureInitialized() {
    if (!_isInitialized) {
      throw Exception('BTreeQueueManager not initialized. Call initialize() first.');
    }
  }
  // Add a new patient to the queue
  Future<ActivePatientQueueItem> addPatient(ActivePatientQueueItem patient) async {
    _ensureInitialized();
    
    try {
      // Add to database first
      ActivePatientQueueItem addedPatient = await _queueService.addPatientDataToQueue({
      ...
      debugPrint('BTreeQueueManager: Added patient ${addedPatient.patientName} to queue');
      return addedPatient;
    } catch (e) {
  ...
  Future<bool> removePatient(String queueEntryId) async {
    ...
    try {
      ...
      ActivePatientQueueItem? patient = await _queueService.getQueueItem(queueEntryId);
      ...
      if (removed) {
        ...
        debugPrint('BTreeQueueManager: Removed patient ${patient.patientName} from queue');
      }
  ...
  Future<bool> updatePatientStatus(String queueEntryId, String newStatus) async {
    _ensureInitialized();
    
    try {
      // Update in database first
      bool updated = await _queueService.updatePatientStatusInQueue(queueEntryId, newStatus);
      
      if (updated) {
        // Get updated patient data
        ActivePatientQueueItem? updatedPatient = await _queueService.getQueueItem(queueEntryId);
        if (updatedPatient != null) {
          // Update in B-Tree
          _bTreeQueue.update(updatedPatient);
          debugPrint('BTreeQueueManager: Updated patient ${updatedPatient.patientName} status to $newStatus');
        }
      }
  ...
  List<ActivePatientQueueItem> getFilteredItems({
    ...
    List<ActivePatientQueueItem> items = todayOnly 
        ? getTodayItems() 
        : getAllItems();
  ...
  List<ActivePatientQueueItem> _sortItemsByPriority(List<ActivePatientQueueItem> items) {
    items.sort((a, b) {
  ...
  Future<bool> validateConsistency() async {
    ...
    try {
      List<ActivePatientQueueItem> dbItems = await _queueService.getActiveQueueItems();
      List<ActivePatientQueueItem> treeItems = getAllItems();
      
      // Check if counts match
      if (dbItems.length != treeItems.length) {
        debugPrint('BTreeQueueManager: Consistency check failed - count mismatch');
        return false;
      }
      
      // Check if all items exist in both
      for (var dbItem in dbItems) {
        if (!treeItems.any((treeItem) => treeItem.queueEntryId == dbItem.queueEntryId)) {
          debugPrint('BTreeQueueManager: Consistency check failed - missing item ${dbItem.queueEntryId}');
          return false;
        }
      }
