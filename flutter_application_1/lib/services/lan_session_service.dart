...
class LanSessionService {
  ...
  // Getters
  static Stream<Map<String, dynamic>> get sessionUpdates => _sessionUpdates.stream;
  static bool get isServerRunning => _isServerRunning;
  static Map<String, UserSession> get activeSessions => Map.unmodifiable(_activeSessions);
  ...
  static Future<bool> startSessionServer({int port = _defaultSessionPort}) async {
    try {
      ...
      debugPrint('Session server started on port $port with token: $_serverToken');
  ...
  static Future<UserSession?> registerUserSession({
    ...
    try {
      ...
      if (existingSession != null) {
        throw Exception('User "$username" is already logged in on device "${existingSession.deviceName}"');
      }
  ...
  static Future<void> _handleEndSession(HttpRequest request, String sessionId) async {
