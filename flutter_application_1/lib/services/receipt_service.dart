...
class ReceiptService {
  static Future<Uint8List> generateReceiptPdfBytes(
      Map<String, dynamic> receiptDetails) async {
    ...
    final paymentDate =
        receiptDetails['paymentDate'] as DateTime? ?? DateTime.now();
    final totalAmount =
        (receiptDetails['totalAmount'] as num?)?.toDouble() ?? 0.0;
    final amountPaid =
        (receiptDetails['amountPaid'] as num?)?.toDouble() ?? 0.0;
    ...
    pdf.addPage(pw.Page(build: (context) {
      return pw
        ...
        pw.Text(
            'Date: ${DateFormat('yyyy-MM-dd hh:mm a').format(paymentDate)}'),
        pw.Divider(height: 30),
        ...
        pw.Row(mainAxisAlignment: pw.MainAxisAlignment.end, children: [
          pw.Text('Total: PHP${totalAmount.toStringAsFixed(2)}',
